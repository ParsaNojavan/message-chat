...
import { BadRequestException, ConflictException, ForbiddenException, HttpStatus, Inject, Injectable, NotFoundException, UnauthorizedException } from '@nestjs/common';
...
export class GroupRtcService {
  ...
  private getObjectIdOrString(value: string): Array<string | Types.ObjectId> {
  ...
  async createToken(roomId: string, userId: string) {
    ...
    const roomMember = await this.memberModel.findOne({
      roomId: { $in: this.getObjectIdOrString(nRoomId) },
      userId: { $in: this.getObjectIdOrString(nUserId) },
    });
    ...
    const otherMembers = await this.memberModel
      .find({
        roomId: { $in: this.getObjectIdOrString(nRoomId) },
        userId: { $nin: this.getObjectIdOrString(nUserId) },
      })
      .select('userId')
      .lean();

    const targetUserIds = otherMembers
  ...
  async handleDecline(roomId: string, userId: string): Promise<{ callEnded: boolean }> {
    ...
    // پاک کردن وضعیت کاربر رد کننده از Redis
    ...
    if (room.type === ChatType.GROUP) {
      ...
      // در تماس دونفره رد کردن به معنای اتمام کامل تماس است
      await this.redis.del(activeRoomKey);
      await this.redis.del(participantsKey);
  ...
  async getCalls(userId: string, page: number = 1, limit: number = 20): Promise<DataResultDto<any>> {
    ...
    const userMemberships = await this.memberModel
      .find({ userId: { $in: this.getObjectIdOrString(nUserId) } })
      .select('roomId')
      .lean();
